...
{
	cSocket = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
	...
	memset(&cAddrIn6, 0, sizeof(cAddrIn6));

	cAddrIn6.sin6_family = AF_INET6;
	inet_pton(AF_INET6, address, &cAddrIn6.sin6_addr);
	cAddrIn6.sin6_port = htons(port);

	if (connect(cSocket, (SOCKADDR*)&cAddrIn6, sizeof(cAddrIn6)) == SOCKET_ERROR)
	{
